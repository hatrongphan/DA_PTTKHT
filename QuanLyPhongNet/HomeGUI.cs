...
namespace QuanLyPhongNet
{
    public partial class HomeGUI : Form
    {
        ...
        private void LoadSourceToDRGV()
        {
            ...
            drgvMember.Columns[0].HeaderText = "Mã tài khoản";
        ...
        {
            if (drgvClient.SelectedRows[0] == null)
            {
                MessageBox.Show("Lỗi rồi bà con ơi !");
                return;
            }
            int index = drgvClient.SelectedRows[0].Index;
        ...
        {
            Member m = (Member)drgvMember.CurrentRow.DataBoundItem;
            this.objWriter.DeleteMember(m.MemberID);
            drgvMember.DataSource = objReader.GetAllMembers();
            MessageBox.Show("Xóa thành công tài khoản " + m.AccountName + " thành công !", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }
}
